/**
 * BUILD10.14.4 - RUN PAGE CHECK (HTTP-callable)
 * 
 * HTTP endpoint for the admin panel "Run Now" button.
 * Contains the same monitoring logic as check-page-changes.js
 * but WITHOUT the schedule config, making it callable via HTTP.
 * 
 * Netlify scheduled functions cannot be invoked via URL:
 * https://docs.netlify.com/build/functions/scheduled-functions/
 * "You can't invoke scheduled functions directly with a URL."
 * 
 * This function exists as the HTTP-callable counterpart.
 * check-page-changes.js handles the daily 9am CST scheduled run.
 * run-page-check.js handles on-demand browser "Run Now" requests.
 * 
...
// NO schedule config - this is a regular HTTP function

export default async (event, context) => {
  const startTime = Date.now();
  const timestamp = new Date().toISOString();
  console.log('[MONITOR] Starting page change check (manual run)...');
  ...
  try {
    ...
    console.error('[MONITOR] Error in run-page-check:', error);
